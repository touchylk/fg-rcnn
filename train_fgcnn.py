...
head_classifier = nn.fg_classifier(shared_layers, roi_input, cfg.num_rois,nb_classes=10)
legs_classifier = nn.fg_classifier(shared_layers, roi_input, cfg.num_rois,nb_classes=10)
wings_classifier = nn.fg_classifier(shared_layers, roi_input, cfg.num_rois,nb_classes=10)
back_classifier = nn.fg_classifier(shared_layers, roi_input, cfg.num_rois,nb_classes=10)
belly_classifier = nn.fg_classifier(shared_layers, roi_input, cfg.num_rois,nb_classes=10)
breast_classifier = 
        ...
        try:
            ...
            progbar.update(iter_num,
                           [('rpn_cls', np.mean(losses[:iter_num, 0])), ('rpn_regr', np.mean(losses[:iter_num, 1])),
                            ...
                            ('detector_regr', np.mean(losses[:iter_num, 3]))])  # 可视化显示.
